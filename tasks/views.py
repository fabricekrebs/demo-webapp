...
from .chatbot_service import AuthenticationError, ChatbotError, RateLimitError, get_chatbot_service, is_chatbot_enabled
    ...
    def post(self, request, chat_id):
        ...
        try:
            ...
            try:
                ...
                return Response(
                    {"error": "Rate limit exceeded. Please wait before sending another message.", "retry_after": 60},
                    status=429,
                )

            except AuthenticationError as e:
                logger.error(f"Authentication error for chat_id={chat_id}: {e}")
                return Response({"error": "Chatbot authentication failed. Please contact support."}, status=500)
                ...
                bot_msg_obj = ChatMessage.objects.create(chat=chat, message=error_response, is_bot=True)
                return Response(ChatMessageSerializer([user_msg_obj, bot_msg_obj], many=True).data)
...
def chatbot_config(request):
    ...
    try:
        config = {"enabled": is_chatbot_enabled(), "status": "unknown"}
        ...
        if config["enabled"]:
            try:
                ...
                config.update({"status": "error", "error": str(e)})
        else:
        ...
        return Response({"enabled": False, "status": "error", "error": f"Configuration check failed: {str(e)}"}, status=500)
...
def conversation_summary(request, chat_id):
    ...
    try:
        ...
        if is_chatbot_enabled():
            chatbot_service = get_chatbot_service()
            summary = chatbot_service.get_conversation_summary(chat.azure_thread_id, limit=int(request.GET.get("limit", 20)))
